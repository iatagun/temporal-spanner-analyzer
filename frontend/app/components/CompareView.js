...
export default function CompareTimeRange({ timeRange, timeMin, timeMax, timeMin2, timeMax2, onMinChange, onMaxChange, onMin2Change, onMax2Change, loading, currentGraph, compareGraph, fullGraph, onCompare, onTrends }) {
  ...
  return (
    <div className="border border-border p-4 mb-4 bg-white animate-in">
      <div className="grid md:grid-cols-2 gap-6 mb-3">
        <div>
          <div className="text-xs text-gray-500 mb-1.5">
            Donem A: {formatTime(timeMin)} &mdash; {formatTime(timeMax)}
          </div>
          <div className="relative h-8">
            <input type="range" min={timeRange.min} max={timeRange.max} step={step}
              value={timeMin} onChange={e => { const v = Number(e.target.value); if (v <= timeMax) onMinChange(v); }}
              className="absolute top-2 left-0 w-full h-2 appearance-none bg-transparent pointer-events-auto
                         [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3.5 [&::-webkit-slider-thumb]:h-3.5
                         [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-gray-900 [&::-webkit-slider-thumb]:cursor-pointer [&::-webkit-slider-thumb]:z-10" />
            <input type="range" min={timeRange.min} max={timeRange.max} step={step}
              value={timeMax} onChange={e => { const v = Number(e.target.value); if (v >= timeMin) onMaxChange(v); }}
              className="absolute top-2 left-0 w-full h-2 appearance-none bg-transparent pointer-events-auto
                         [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3.5 [&::-webkit-slider-thumb]:h-3.5
                         [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-gray-500 [&::-webkit-slider-thumb]:cursor-pointer [&::-webkit-slider-thumb]:z-10" />
        ...
        <div>
          <div className="text-xs text-gray-500 mb-1.5">
            Donem B: {formatTime(timeMin2)} &mdash; {formatTime(timeMax2)}
          </div>
          <div className="relative h-8">
            <input type="range" min={timeRange.min} max={timeRange.max} step={step}
              value={timeMin2} onChange={e => { const v = Number(e.target.value); if (v <= timeMax2) onMin2Change(v); }}
              className="absolute top-2 left-0 w-full h-2 appearance-none bg-transparent pointer-events-auto
                         [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3.5 [&::-webkit-slider-thumb]:h-3.5
                         [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-gray-900 [&::-webkit-slider-thumb]:cursor-pointer [&::-webkit-slider-thumb]:z-10" />
            <input type="range" min={timeRange.min} max={timeRange.max} step={step}
              value={timeMax2} onChange={e => { const v = Number(e.target.value); if (v >= timeMin2) onMax2Change(v); }}
              className="absolute top-2 left-0 w-full h-2 appearance-none bg-transparent pointer-events-auto
                         [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3.5 [&::-webkit-slider-thumb]:h-3.5
                         [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-gray-500 [&::-webkit-slider-thumb]:cursor-pointer [&::-webkit-slider-thumb]:z-10" />
            <div className="absolute top-2 left-0 right-0 h-2 bg-gray-200 rounded-full pointer-events-none">
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        <button onClick={() => onTrends(fullGraph)}
          disabled={loading || !fullGraph}
          className="px-4 py-1.5 border border-gray-300 text-xs text-gray-600 rounded hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
          Trendler
        </button>
      </div>
    </div>
...
export function CompareResult({ data }) {
  ...
  return (
    <div className="animate-in">
      <div className="border border-gray-200 bg-gray-50 p-4 mb-5 text-sm text-gray-700">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-px bg-gray-100 border border-gray-200 mb-6">
        <MetricCard label="Ortak Kelime" value={`%${c.vertex_overlap_pct}`} />
        <MetricCard label="Ortak Baglanti" value={`%${c.edge_overlap_pct}`} />
        <MetricCard label="Kazanan" value={c.savings_compare} />
        <MetricCard label="Klik A" value={c.clique_count_1} />
        <MetricCard label="Klik B" value={c.clique_count_2} />
        <MetricCard label="Benzerlik" value={c.clique_jaccard} />
      </div>

      <div className="grid md:grid-cols-2 gap-4 mb-6">
        <div className="border border-gray-200 p-4">
          <div className="text-xs text-gray-500 mb-3">Donem A</div>
          <div className="grid grid-cols-2 gap-px bg-gray-100 border border-gray-100 mb-4">
