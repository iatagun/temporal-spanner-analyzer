...
export default function AnalyzerRoute() {
  // useSearchParams() opts this route out of static rendering unless
  // wrapped in Suspense (Next.js requirement) -- the inner component is
  // already 100% client-rendered/stateful, so this boundary is a pure
  // formality, never actually shows the fallback in practice.
  return (
...
function AnalyzerApp() {
  ...
  // Landing page's "Örnek Veri ile Dene" CTA links here with ?sample=1 so
  // it can jump straight into a loaded demo instead of dropping the user
  // on an empty upload screen. autoSampleRef guards against firing twice
  // under React 18/19 strict-mode's dev-only double-invoke of effects.
